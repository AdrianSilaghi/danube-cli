...
import { formatUnknownCommand, wantsJsonOutput } from './lib/command-resolution.js';

/**
 * Build the command tree.
 *
 * Separate from the entry point so the tree can be inspected without running
 * the CLI — the unknown-command tests assert against the REAL commands rather
 * than a hand-written copy that would drift the first time one is added.
 */
export function buildProgram(): Command {
  ...
  // A net for anything the pre-parse walk in the entry point deliberately
